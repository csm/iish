...
fn evaluate_simple_command(
    ...
    // `<`, `>`/`2>` onto a real file, process substitution as a redirect
    // target, ...) is denied below. A single stdin heredoc is collected
    // separately for the native `cat` and `read` implementations.
    ...
    if let Some(doc) = heredoc {
        if append_target.is_some() {
            return deny("combining a here-document with `>>` is not implemented yet");
        }
        // `cat <<EOF` prints the supplied stdin; `read -r NAME <<EOF`
        // consumes its first line into NAME. Both stay native, so the
        // here-document never crosses a subprocess or shell boundary.
        return match name.as_str() {
            "cat" if args.is_empty() => evaluate_cat_heredoc(doc, redirects.stdout),
            "read" => evaluate_read_heredoc(&args, doc),
            _ => deny("here-documents are only implemented for bare `cat` and `read [-r] NAME`"),
        };
    }
...
fn evaluate_cat_heredoc(doc: &ast::IoHereDocument, dest: StdoutDest) -> Verdict {
    let text = match heredoc_text(doc) {
        Ok(text) => text,
        Err(reason) => return deny(reason),
    };
    allow(
...
/// Materialize a here-document whose contents need no shell expansion.
fn heredoc_text(doc: &ast::IoHereDocument) -> Result<String, String> {
    ...
    if doc.requires_expansion && text.contains(['$', '`']) {
        return Err("a here-document containing expansions is not implemented yet".into());
    }
    Ok(text)
}

/// `read [-r] NAME <<EOF`: assign the first input line to NAME. With
/// one destination variable, shell `read` keeps the whole line except
/// leading/trailing IFS whitespace; `-r` is accepted by
/// `read_variable_name` and is the form generated by cargo-dist.
fn evaluate_read_heredoc(args: &[String], doc: &ast::IoHereDocument) -> Verdict {
    ...
    let Some(line) = text.lines().next() else {
        return allow(
            "reads EOF from an empty here-document",
            Action::Test { result: false },
        );
    };
    allow(
        format!("reads the first here-document line into `{name}`; nothing else"),
        Action::Assign {
            assignments: vec![(name, line.trim_matches([' ', '\t']).to_string())],
        },
    )
}
...
fn read_variable_name(args: &[String]) -> Result<String, String> {
    ...
    let [name] = names.as_slice() else {
        return Err("`read` with other than exactly one variable name is not supported".into());
    };
    Ok(name.clone())
}
